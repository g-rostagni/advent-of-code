...
function number_of_wins(f)
	...
	for line in eachline(f)							# loop over each card
		card = split(line, [':','|'])
		winning_nums = split(card[2], r" +")				# retrieve the winning numbers
		my_nums = split(card[3], r" +")					# retrieve our numbers
		num = count(n->(!isempty(n) && n in winning_nums), my_nums)	# count how many of our numbers are also winning numbers
...
const nwins::Vector{Int64} = number_of_wins("4-data")

score::Int64 = 0			# define the score (part 1)
...
for i in 1:length(nwins)			# loop over all the cards
	...
	global score += 2^(wins - 1)		# add the score of the card to the total
	for j in 1:wins				
		global ncards[i+j] += ncards[i]	# add the number of copies that we won to the following cards
	end
end
